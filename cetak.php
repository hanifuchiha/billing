...
if (!empty($_FILES['logo']['name'])) {
    if ($_FILES['logo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, ['png','jpg','jpeg'])) {
            ...
            $logo_path ='https://quenbytekniksejahtera.com/dokumen/logo/' . $logo_name;
...
if (!empty($_FILES['bg']['name'])) {
    if ($_FILES['bg']['error'] === UPLOAD_ERR_OK) {
        ...
        $dest_bg = $uploadDir . '/' . $bg_name;
          $bg_path ='https://quenbytekniksejahtera.com/dokumen/logo/' . $bg_name;
...
foreach ($_POST['selected'] as $i => $item_json) {
 ...
 // bawa logo_path dan bg_path
    ...
    // gabungkan vouchers
    foreach ($data['vouchers'] as $v) $all_vouchers[] = $v;
}
...
// Update status_cetak di database untuk voucher yang dicetak
require_once '../koneksidb.php'; // pastikan koneksi $conn tersedia

if (!empty($all_vouchers)) {
    $usernames = array();
    foreach ($all_vouchers as $v) {
        if (!empty($v['user'])) {
            $usernames[] = "'" . mysqli_real_escape_string($conn, $v['user']) . "'";
        }
    }
    if (!empty($usernames)) {
        $usernames_str = implode(",", $usernames);
        $sql_update = "UPDATE voucher SET status_cetak=1 WHERE voucher IN ($usernames_str)";
...
foreach($_POST as $k => $v){
    ...
    if(in_array($k, ['desain'])) continue; // skip ketiga variabel
    if(is_array($v)){
        foreach($v as $val){
...
echo '<input type="hidden" name="vouchers" value="'.htmlspecialchars($data['vouchers'] ?? '').'">';
