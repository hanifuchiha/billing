            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        ...
                        <p class="text-muted small mb-0">Semua koneksi VPN di sini menggunakan protokol <strong>SSTP</strong> (port 4433) ke router utama. VPN ini dipakai untuk remote/manage perangkat MikroTik pelanggan dari jarak jauh lewat tunnel terenkripsi, dengan hingga 5 port layanan (4 port standar + 1 port custom untuk kebutuhan khusus) yang di-forward otomatis sesuai isian saat menambahkan.</p>
</div>
...
<br>
                        ...
                        <form method="POST" action="proses/cek_sstp_server.php" class="d-inline">
                            <input type="hidden" name="page" value="vpnadmin">
                                    ...
                                    <div class="modal-body">
                                        <form method="POST" action="proses/belivpn.php">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="form-text mb-2">Port-port ini akan di-forward otomatis dari server ke MikroTik tujuan sesuai isian di bawah. 4 port pertama default mengikuti port bawaan MikroTik (boleh diubah), ditambah 1 port custom opsional untuk kebutuhan khusus lain kalau diperlukan.</div>
                                                <div class="row g-2">
...
// Query SQL
$sql = "SELECT * FROM pelanggan WHERE BRAND LIKE 'VPN'";
...
$scriptSstp .= "add dst-host=*.$domainaccept action=allow\n";
      ...
      // dijalankan sendiri oleh pelanggan di MikroTik-nya kalau mau diteruskan lagi
          ...
          <p class="mb-1 text-light"><strong>VPN IP : </strong><?= $ip_address ?>  ( Gunakan ini untuk add server kolom IP, port SSTP 4433 ) </p>
            ...
            <ul class="mb-1 text-light ps-3">
              <?php foreach ($pairs as $pair): ?>
                <li>Port publik <strong><?= htmlspecialchars($pair['in']) ?></strong> &rarr; Port <?= htmlspecialchars($pair['out'] ?? '-') ?> di MikroTik tujuan</li>
              <?php endforeach; ?>
            </ul>
            <?php endif; ?>
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <p>Copy dan paste script di bawah ini di MikroTik tujuan via Terminal.</p>
              <p class="text-muted small mb-2">Script sama dan kompatibel untuk RouterOS <strong>v6</strong> maupun <strong>v7</strong>.</p>

              <?php
              ob_start();
              ?>
              <h6>Script 1 &mdash; Sambungkan MikroTik ke VPN (SSTP, port 4433)</h6>
              ...
              <?php if ($script2 !== ''): ?>
              <h6>Script 2 &mdash; Forward port layanan ke perangkat lokal (opsional)</h6>
              <p class="text-muted">Jalankan sendiri di MikroTik tujuan kalau ingin meneruskan salah satu port di atas ke perangkat lain di jaringan lokal (contoh: OLT / NVR). Ganti <code>[GANTI IP PERANGKAT ANDA]</code> dan <code>[PORT TUJUAN PERANGKAT ANDA]</code> sesuai perangkat tujuan.</p>
