...
$sql = "SELECT * FROM faspay WHERE pemilik='$ownerbilling'";
...
if ($query === false) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data faspay (query error)',
        'debug' => [
...
if (!$data) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data faspay (data tidak ditemukan)',
        'debug' => [
            'ownerbilling' => $ownerbilling,
            'db_selected' => $dbName,
            'sql' => $sql,
...
$faspayMerchantId = $data['merchant_id'];
$faspayUserId = $data['user_id'];
$faspayPassword = $data['password'];
$pajak = $data['pajak'];
$apihotspot = 'NO'; // Faspay belum mendukung integrasi hotspot voucher

//////////////////////////////BACA PAYLOAD NOTIFIKASI FASPAY//////////////////////////////
$jsontest = file_get_contents('php://input');
$arr = json_decode($jsontest, true);
if (!is_array($arr) || empty($arr)) {
    // Faspay umumnya mengirim sebagai application/x-www-form-urlencoded
    $arr = $_POST;
}
if (!is_array($arr) || empty($arr)) {
    exit(json_encode([
        ...
        'response_code' => '01',
        'response_desc' => 'Invalid payload',
    ]));
}

$faspayTrxId = (string)($arr['trx_id'] ?? '');
$faspayBillNo = (string)($arr['bill_no'] ?? '');
$faspayStatusCode = (string)($arr['payment_status_code'] ?? '');
$faspaySignature = (string)($arr['signature'] ?? '');

//////////////////////////////VERIFIKASI FASPAY: sha1(md5(user_id+password+bill_no+payment_status_code))//////////////////////////////
$faspayExpectedSignature = sha1(md5($faspayUserId . $faspayPassword . $faspayBillNo . $faspayStatusCode));

if ($faspayTrxId === '' || $faspayBillNo === '' || $faspaySignature === '' || !hash_equals($faspayExpectedSignature, $faspaySignature)) {
    exit(json_encode([
        'response' => 'Payment Notification',
        'trx_id' => $faspayTrxId,
        'merchant_id' => $faspayMerchantId,
        'bill_no' => $faspayBillNo,
        'response_code' => '01',
...
// ============ KIRIM ACK KE FASPAY (format wajib, atau akan di-retry 3x) ============
...
//////////////////////////////NO REF DARI FASPAY//////////////////////////////
// payment_status_code: 2=Success, 3=Failed, 7=Expired, 8=Cancelled
$merchant_ref = $faspayBillNo;
$cekstatus = ($faspayStatusCode === '2') ? 'PAID' : (($faspayStatusCode === '7') ? 'EXPIRED' : 'UNPAID');
$amount = (float)($arr['payment_total'] ?? $arr['bill_total'] ?? 0);
$payment_method = (string)($arr['payment_channel'] ?? 'Faspay');
$payment_method_code = (string)($arr['payment_channel_uid'] ?? '');
$customer_name = '';
$customer_phone = '';
$payment_link = '#';
...
//SEBAGAI PATOKAN
$invoiceref = $faspayBillNo;




$history[] = "[ callback faspay - " . date('Y-m-d H:i:s') . " ] Faspay cek kode ref transaksi $invoiceref";
callbackLogStep($history, $history_file, 'FASPAY_REF_CHECK', "Reference: $invoiceref | Status: " . $cekstatus);
                    ...
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
                                ...
                                if($AUTHMODE=='RADIUS MODE' | $AUTHMODE=='MULTI MODE'  )
                                {
                                                                                    ...
                                                                                    if ($PASSWORDPELANGGAN === '' || $PASSWORDPELANGGAN === null) {
                                                                                        $history[] = "[ callback faspay - " . date('Y-m-d H:i:s') . " ] PERINGATAN: password PPPoE kosong untuk $USERNAMETRANASAKSI, RADIUS TIDAK diaktifkan otomatis.";
                                                                                    } else {
                                                                                        radiusSyncSingleCustomerNow($USERNAMETRANASAKSI, $PASSWORDPELANGGAN, $paketRow, true, radiusGetGlobalSettings($conn));
                                                                                    }

                                                                                $history[] = "[ callback faspay - " . date('Y-m-d H:i:s') . " ] Faspay berhasil aktifkan OTOMATIS $USERNAMETRANASAKSI $NAMAPELANGGAN $PAKETPELANGGAN";
                                                                                // Simpan ke file history
                                                                                file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
