...
$sql = "SELECT * FROM tripay WHERE pemilik='$ownerbilling'";
...
if ($query === false) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data tripay (query error)',
        'debug' => [
...
if (!$data) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data tripay (data tidak ditemukan)',
        'debug' => [
            'ownerbilling' => $ownerbilling,
            'db_selected' => $dbName,
            'sql' => $sql,
...
$signature = hash_hmac('sha256', $jsontest, $privateKey);
$callbackEvent = $_SERVER['HTTP_X_CALLBACK_EVENT'] ?? '';

if ($callbackSignature === '' || !hash_equals($signature, $callbackSignature)) {
    exit(json_encode([
        'success' => false,
...
$history[] = "[ callback tripay - " . date('Y-m-d H:i:s') . " ] Tripay cek kode ref transaksi $invoiceref";
...
// pelanggan Fixed Due Date bayar 2 Agustus tapi tercatat "September").
// ==========================================
...
if ($periode === '') {
    // Fungsi di atas gagal hitung (mis. TANGGALPASANG kosong/tidak valid) --
                    ...
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
                                ...
                                if($AUTHMODE=='RADIUS MODE' | $AUTHMODE=='MULTI MODE'  )
                                {
                                    ...
                                    if ($PASSWORDPELANGGAN === '' || $PASSWORDPELANGGAN === null) {
                                        $history[] = "[ callback tripay - " . date('Y-m-d H:i:s') . " ] PERINGATAN: password PPPoE kosong untuk $USERNAMETRANASAKSI, RADIUS TIDAK diaktifkan otomatis.";
                                    } else {
