...
if (strtoupper($botname) == 'RANDOM') {
    ...
    if (mysqli_num_rows($query1) > 0) {
        ...
        // Log untuk debug
        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Bot RANDOM dipilih | SELECTED: $botname | WAAPI: $waapi | Total bots: " . count($availableBots);
        ...
        $botpass = '';
        $sender = '';
        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] WARNING: Tidak ada bot untuk pemilik '$ownerbilling'";
        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
    }
} else {
    ...
    $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] DEBUG: Query bot | SQL: $sql1 | Rows: " . mysqli_num_rows($query1);
    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
    ...
    if (mysqli_num_rows($query1) > 0) {
        while ($data1 = mysqli_fetch_array($query1)) {
            ...
            $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] DEBUG: Bot data | namebot: " . $data1['namebot'] . " | addressbot: " . ($data1['addressbot'] ?? 'NULL') . " | password: " . (empty($data1['password']) ? 'EMPTY' : 'SET');
            file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
        }
        // Log untuk debug
        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Bot config loaded | BOTNAME: $botname | WAAPI: " . ($waapi ?? 'UNDEFINED') . " | WAAPI_EMPTY: " . (empty($waapi) ? 'YES' : 'NO');
        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
    } else {
        ...
        $botpass = '';
        $sender = '';
        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] WARNING: Bot '$botname' tidak ditemukan di table botwa";
        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
    }
}
/////////////////////////////////////////////////////////////////////////////////
...
$sql = "SELECT * FROM dompetx WHERE pemilik='$ownerbilling'";
...
if ($query === false) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data dompetx (query error)',
        'debug' => [
...
if (!$data) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data dompetx (data tidak ditemukan)',
        'debug' => [
            'ownerbilling' => $ownerbilling,
            'db_selected' => $dbName,
            'sql' => $sql,
...
$apiKey = $data['api_key'];
$dompetxSecret = !empty($data['secret_key']) ? $data['secret_key'] : $apiKey;
$pajak = $data['pajak'];
$apihotspot = $data['default_auth_mode'];



//////////////////////////////VERIFIKASI DOMPETX//////////////////////////////
// CATATAN: header & skema signature callback DompetX BELUM terkonfirmasi
// eksplisit dari dokumentasi resmi (halaman webhook di docs.dompetx.com
// tidak berhasil diakses saat integrasi ini dibuat -- lihat dompetx_helper.php
// untuk detail lengkap). Diasumsikan SAMA dengan skema signing request biasa:
// HMAC-SHA256(timestamp . '.' . rawBody, secret), dikirim lewat header
// X-DOMPAY-Signature + X-DOMPAY-Timestamp. WAJIB diverifikasi ulang dengan
// akun sandbox DompetX sungguhan sebelum dipakai di production.
// VERIFIKASI SIGNATURE DIMATIKAN ATAS PERMINTAAN -- skema signature webhook
// DompetX tidak pernah berhasil dipastikan cocok (selalu "Invalid callback
// signature" walau secret sudah disamakan manual dengan API Key), jadi
// endpoint ini SEKARANG MENERIMA SEMUA REQUEST tanpa verifikasi asal
// pengirim. Kalau mau diaktifkan lagi nanti, kembalikan blok verifikasi
// HMAC yang sempat ada di sini (lihat riwayat/versi sebelumnya file ini).
...
// Payload webhook DompetX BENAR-BENAR TERKONFIRMASI (dari percobaan nyata
// 2026-08-03): semua field pembayaran (id/amount/status/currency/reference)
// dibungkus di dalam objek "data", bukan flat di level atas seperti contoh
// response "Get Payment Detail" yang dijadikan acuan awal --
// {"data":{"id":...,"amount":...,"status":"paid","reference":"..."},"eventType":"deposit","paymentId":"..."}.
// Fallback ke level atas tetap dijaga kalau-kalau ada varian payload lain.
$dompetxData = is_array($arr['data'] ?? null) ? $arr['data'] : $arr;

// Normalisasi status DompetX ("paid" huruf kecil) ke kosakata yang dipakai
// sisa file ini (persis pola Tripay: string besar "PAID"/"EXPIRED") supaya
// seluruh logic bisnis di bawah (aktivasi layanan, notifikasi WA, dst --
// disalin apa adanya dari callback_tripay.php) tidak perlu diubah satu-satu.
$dompetxRawStatus = strtolower((string) ($dompetxData['status'] ?? ''));
if (in_array($dompetxRawStatus, ['paid', 'success', 'settlement', 'completed'], true)) {
    $arr['status'] = 'PAID';
} elseif (in_array($dompetxRawStatus, ['expired', 'expire'], true)) {
    $arr['status'] = 'EXPIRED';
} elseif ($dompetxRawStatus !== '') {
    $arr['status'] = strtoupper($dompetxRawStatus);
}
// reference (kode unik merchant yang dikirim saat create transaction) dipakai
// sebagai merchant_ref di bawah.
$arr['merchant_ref'] = $dompetxData['reference'] ?? ($dompetxData['merchant_ref'] ?? ($dompetxData['id'] ?? ''));
$arr['total_amount'] = $dompetxData['amount'] ?? ($dompetxData['total_amount'] ?? 0);
$arr['payment_method'] = $dompetxData['type'] ?? ($dompetxData['payment_method'] ?? 'DompetX');
$arr['payment_method_code'] = $dompetxData['type'] ?? ($dompetxData['payment_method_code'] ?? '');
$arr['customer_name'] = $dompetxData['metadata']['customer_name'] ?? ($dompetxData['customer_name'] ?? '');
$arr['customer_phone'] = $dompetxData['metadata']['customer_phone'] ?? ($dompetxData['customer_phone'] ?? '');
$arr['checkout_url'] = $dompetxData['checkout_url'] ?? ($arr['checkout_url'] ?? '#');
// reference dipakai lagi di bawah via $arr["reference"] (patokan $invoiceref) --
// isi juga di level atas $arr supaya konsisten dengan field lain yang sudah
// dinormalisasi ke $arr di atas.
$arr['reference'] = $dompetxData['reference'] ?? ($arr['reference'] ?? '');

// ============ KIRIM RESPONSE HTTP 200 OK LANGSUNG KE TRIPAY ============
...
// Ekstrak SKU dari callback
$sku = $arr["items"][0]["sku"] ?? '';


//SEBAGAI PATOKAN
$invoiceref = $arr["reference"] ?? '';




$history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Tripay cek kode ref transaksi $invoiceref";
callbackLogStep($history, $history_file, 'TRIPAY_REF_CHECK', "Reference: $invoiceref | Status: " . $cekstatus);
...
if (!in_array($PAKET, ["TOPUP", "TOPUP_MITRA"])) {
    ...
    if (mysqli_num_rows($query_server) == 0) {
        // Log error ke history
        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] ERROR: Server tidak ditemukan untuk area '$AREAPELANGGAN' - transaksi $invoiceref";
        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
...
if ($LAYANAN == "PPPOE") {
            ...
            if (!$log_recorded) {
                ...
                $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Tripay berhasil notif BOT wahtsapp dan email pelanggan | WAAPI: $waapi | BOTNAME: $botname | HTTP: $httpCode | CURL_ERROR: $curlError | BOT_RESPONSE: $responseLog | MSG: $text";
            ...
            if ($conn->query($sql1) === TRUE) {
                ...
                if ($conn->query($sql11) === TRUE) {
                    ...
                    $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Tripay berhasil mencatat transaksi OTOMATIS  '$tanggalbayar','$periode','$USERNAMETRANASAKSI','$NAMAPELANGGAN','$PAKETPELANGGAN','$HARGAPELANGGAN','BERHASIL','$invoiceref','$user100',''";
                    // Simpan ke file history
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
                                ...
                                if($AUTHMODE=='API MODE' | $AUTHMODE=='MULTI MODE'  )
                                {
                                                    ...
                                                    $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Tripay berhasil aktifkan OTOMATIS $USERNAMETRANASAKSI $NAMAPELANGGAN $PAKETPELANGGAN";
                                                    // Simpan ke file history
                                                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
                                ...
                                if($AUTHMODE=='RADIUS MODE' | $AUTHMODE=='MULTI MODE'  )
                                {
                                    ...
                                    if ($PASSWORDPELANGGAN === '' || $PASSWORDPELANGGAN === null) {
                                        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] PERINGATAN: password PPPoE kosong untuk $USERNAMETRANASAKSI, RADIUS TIDAK diaktifkan otomatis.";
                                    } else {
            ...
            if ($conn->query($sql10) === TRUE) {
                ...
                if (!$notification_sent) {
                    ...
                    if ($conn->query($sql13) === TRUE) {
                        ...
                        $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] ERROR: Gagal update transaksi topup $invoiceref - " . $conn->error;
                        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
...
// Response sudah dikirim di awal callback, hanya log saja di sini
            $history[] = "[ callback dompetx - " . date('Y-m-d H:i:s') . " ] Background processing PPPOE selesai untuk $USERNAMETRANASAKSI";
            file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
