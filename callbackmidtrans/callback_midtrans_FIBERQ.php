...
$BRANDPELANGGAN = end($parts);

// Ambil data Midtrans dari DB
$sql = "SELECT * FROM midtrans WHERE pemilik='$BRANDPELANGGAN'";
$query = mysqli_query($conn, $sql);
$data = mysqli_fetch_array($query);
if (!$data) {
	die(json_encode(["status" => "error", "message" => "Gagal mengambil data midtrans"]));
}
...
$signature_key = $arr['signature_key'] ?? '';
$order_id = $arr['order_id'] ?? '';
$status_code = $arr['status_code'] ?? '';
$gross_amount = $arr['gross_amount'] ?? 0;
...
$fraud_status = $arr['fraud_status'] ?? '';

// Mapping status dan variabel utama
$invoiceref = $order_id;
$cekstatus = ($transaction_status === 'capture' || $transaction_status === 'settlement') ? 'PAID' : ($transaction_status === 'pending' ? 'PENDING' : 'FAILED');
...
$history[] = "[ callback midtrans - " . date('Y-m-d H:i:s') . " ] Midtrans cek kode ref transaksi $invoiceref";
...
if (empty($lastUsageMonth) || empty($lastUsageYear)) {
    ...
    if ($currentDay >= $tanggal_awal_tutup_buku && $currentDay <= $tanggal_akhir_tutup_buku) {
        // Tanggal 16–24: prorate bulan ini + full bulan depan
        ...
        // Tanggal 6–15: tagih prorate bulan ini sampai tanggal 24
        ...
        // Tanggal 25–akhir bulan: tagih full bulan depan
        $tagihanDetail[] = [
            'keterangan' => 'Tagihan Bulan ' . getMonthName($currentMonth + 1, $currentYear),
            'harga' => $paketHarga
        ];
...
// di sini pakai heuristik tutup-buku generik ($tanggal_awal_tutup_buku/akhir) yang
// tidak sadar TIPE_TEMPO/Periode Tercatat sama sekali -- bisa salah bulan (mis.
// pelanggan Fixed Due Date bayar 7 Agustus tapi tercatat "Juli").
// ==========================================
...
if ($sekali == 0) {
    ...
    if ($cekstatus == "PAID") {
        if ($LAYANAN == "PPPOE") {
            ...
            $history[] = "[ callback midtrans - " . date('Y-m-d H:i:s') . " ] Midtrans berhasil notif BOT wahtsapp dan email pelanggan  $text";
            ...
            if ($conn->query($sql1) === TRUE) {
                ...
                if ($conn->query($sql11) === TRUE) {
                    ...
                    $history[] = "[ callback midtrans - " . date('Y-m-d H:i:s') . " ] Midtrans berhasil mencatat transaksi OTOMATIS  '$tanggalbayar','$periode','$USERNAMETRANASAKSI','$NAMAPELANGGAN','$PAKETPELANGGAN','$HARGAPELANGGAN','BERHASIL','$invoiceref','$BRANDPELANGGAN',''";
                    // Simpan ke file history
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
...
if($AUTHMODE=='API MODE' | $AUTHMODE=='MULTI MODE'  )
{
                    ...
                    $API->comm(
                     ...
                     $history[] = "[ callback midtrans - " . date('Y-m-d H:i:s') . " ] Midtrans berhasil aktifkan OTOMATIS  '$USERNAMETRANASAKSI','$NAMAPELANGGAN','$PAKETPELANGGAN'";
                    // Simpan ke file history
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
...
if($AUTHMODE=='RADIUS MODE' | $AUTHMODE=='MULTI MODE'  )
{
                                    ...
                                    if ($PASSWORDPELANGGAN === '' || $PASSWORDPELANGGAN === null) {
                                        $history[] = "[ callback midtrans - " . date('Y-m-d H:i:s') . " ] PERINGATAN: password PPPoE kosong untuk $USERNAMETRANASAKSI, RADIUS TIDAK diaktifkan otomatis.";
                                    } else {
        ...
        if ($LAYANAN == "HOTPSOT") {
            ...
            $area = $pecah[5];
                                ...
                                function convertToSeconds($limitUptime)
                                {
                                    ...
                                    if ($return_var === 0) {
                                        restartFreeradius();
                                        echo "✅ RADIUS Success Start <br>" . implode("<br>", $output);
                                        ...
                                        $sql = "INSERT INTO `transaksi`(`TANGGALBAYAR`,`STATUS`, `IDPEL`, `NAMA`, `PAKET`, `HARGA`, `BUKTI`, `CEK`, `PEMILIK`) 
                                                VALUES ('$tanggalbayar','BERHASIL','$USERNAMETRANASAKSI','$NAMATRANASAKSI','$PAKET','$voucher_amount','$invoiceref','', '$BRANDPELANGGAN')";
                                        $conn->query($sql);
                                        ...
                                        echo "❌ Gagal menambahkan user ke file FreeRADIUS.";
                                    }
