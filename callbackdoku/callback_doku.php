...
$sql = "SELECT * FROM doku WHERE pemilik='$ownerbilling'";
...
if ($query === false) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data doku (query error)',
        'debug' => [
...
if (!$data) {
    exit(json_encode([
        'success' => false,
        'message' => 'Gagal mengambil data doku (data tidak ditemukan)',
        'debug' => [
            'ownerbilling' => $ownerbilling,
            'db_selected' => $dbName,
            'sql' => $sql,
...
$dokuClientId = $data['client_id'];
$dokuSecretKey = $data['secret_key'];
$pajak = $data['pajak'];
$apihotspot = 'NO'; // DOKU belum mendukung integrasi hotspot voucher

//////////////////////////////VERIFIKASI DOKU (HMAC-SHA256, header Client-Id/Request-Id/Request-Timestamp/Digest/Signature)//////////////////////////////
$jsontest = file_get_contents('php://input');

$dokuClientIdHeader = $_SERVER['HTTP_CLIENT_ID'] ?? '';
$dokuRequestId = $_SERVER['HTTP_REQUEST_ID'] ?? '';
$dokuRequestTimestamp = $_SERVER['HTTP_REQUEST_TIMESTAMP'] ?? '';
$dokuSignatureHeader = $_SERVER['HTTP_SIGNATURE'] ?? '';
$dokuRequestTarget = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH) ?: '/';

$dokuDigest = base64_encode(hash('sha256', $jsontest, true));
$dokuRawSignature = "Client-Id:$dokuClientIdHeader\n"
    . "Request-Id:$dokuRequestId\n"
    . "Request-Timestamp:$dokuRequestTimestamp\n"
    . "Request-Target:$dokuRequestTarget\n"
    . "Digest:$dokuDigest";
$dokuExpectedSignature = 'HMACSHA256=' . base64_encode(hash_hmac('sha256', $dokuRawSignature, $dokuSecretKey, true));

if (
    $dokuClientId === '' || $dokuClientIdHeader === '' || $dokuClientId !== $dokuClientIdHeader ||
    $dokuSignatureHeader === '' || !hash_equals($dokuExpectedSignature, $dokuSignatureHeader)
) {
    exit(json_encode([
        'success' => false,
...
// ============ KIRIM RESPONSE HTTP 200 OK LANGSUNG KE DOKU ============
// DOKU mensyaratkan notifikasi di-ack cepat (2xx) sebelum logika bisnis berat dijalankan.
...
//////////////////////////////NO REF DARI DOKU//////////////////////////////
$dokuStatus = strtoupper((string)($arr['transaction']['status'] ?? ''));
$merchant_ref = (string)($arr['order']['invoice_number'] ?? '');
$cekstatus = ($dokuStatus === 'SUCCESS') ? 'PAID' : 'UNPAID';
$amount = (float)($arr['order']['amount'] ?? 0);
$payment_method = (string)($arr['payment']['type'] ?? 'DOKU');
$payment_method_code = (string)($arr['channel']['id'] ?? '');
$customer_name = (string)($arr['customer']['name'] ?? '');
$customer_phone = (string)($arr['customer']['phone'] ?? '');
$payment_link = '#';
...
//SEBAGAI PATOKAN
$invoiceref = (string)($arr['order']['invoice_number'] ?? '');




$history[] = "[ callback doku - " . date('Y-m-d H:i:s') . " ] DOKU cek kode ref transaksi $invoiceref";
callbackLogStep($history, $history_file, 'DOKU_REF_CHECK', "Reference: $invoiceref | Status: " . $cekstatus);
                    ...
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
                                ...
                                if($AUTHMODE=='RADIUS MODE' | $AUTHMODE=='MULTI MODE'  )
                                {
                                                                                    ...
                                                                                    if ($PASSWORDPELANGGAN === '' || $PASSWORDPELANGGAN === null) {
                                                                                        $history[] = "[ callback doku - " . date('Y-m-d H:i:s') . " ] PERINGATAN: password PPPoE kosong untuk $USERNAMETRANASAKSI, RADIUS TIDAK diaktifkan otomatis.";
                                                                                    } else {
                                                                                        radiusSyncSingleCustomerNow($USERNAMETRANASAKSI, $PASSWORDPELANGGAN, $paketRow, true, radiusGetGlobalSettings($conn));
                                                                                    }

                                                                                $history[] = "[ callback doku - " . date('Y-m-d H:i:s') . " ] Doku berhasil aktifkan OTOMATIS $USERNAMETRANASAKSI $NAMAPELANGGAN $PAKETPELANGGAN";
                                                                                // Simpan ke file history
                                                                                file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
