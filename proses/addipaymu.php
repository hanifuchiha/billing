...
$va = $_POST['ip_va'] ?? '';
$apiKey = $_POST['ip_api_key'] ?? '';
$server = $_POST['ip_server'] ?? '';
$ip_pajak = $_POST['ip_pajak'] ?? '';
...
$ip_bhps_uso = '0';
$authMode = $_POST['ip_default_auth_mode'] ?? 'API MODE';
// Pakai $ceknama (owner), bukan $_SESSION['USERNAME'], karena semua query
// tampilan/list iPaymu di paymentset.php & lookup runtime di portal_bayar.php
...
$callbackUrl = "https://$domain/crm/billing/callbackipaymu/callback_ipaymu_$ceknama.php";
$returnUrl = "https://$domain/crm/billing/broadband/portallogin.php";

if ($va === '' || $apiKey === '' || $callbackUrl === '' || $returnUrl === '' || $server === '' || $pemilik === '' || $authMode === '') {
    respond_and_exit('Semua field wajib diisi');
}

if ($ip_pajak !== '' && (!is_numeric($ip_pajak) || $ip_pajak < 0)) {
...
$folder = '../callbackipaymu/';
$allowFiles = ['callback_ipaymu.php'];
foreach ($allowFiles as $filename) {
    ...
    if (!file_exists($baru)) {
        if (copy($file, $baru)) {
            ...
            $history[] = "[ " . (!empty($asistant_name) ? $asistant_name : $ceknama) . " - " . date('Y-m-d H:i:s') . " ] Berhasil mengatur akun payment gateway iPaymu untuk $pemilik";
...
// Auto-install tabel `ipaymu` kalau belum ada (mis. server yg belum sempat
// jalankan migrations/2026-07-17_add_ipaymu_doku_faspay.sql).
@mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `ipaymu` (
...
$vaEsc = mysqli_real_escape_string($conn, $va);
$apiKeyEsc = mysqli_real_escape_string($conn, $apiKey);
$pajakEsc = mysqli_real_escape_string($conn, $ip_pajak !== '' ? $ip_pajak : '0');
$bhpsUsoEsc = mysqli_real_escape_string($conn, $ip_bhps_uso);
...
$sql = "INSERT INTO `ipaymu`(`server`, `pemilik`, `va`, `api_key`, `pajak`, `bhps_uso`, `default_auth_mode`, `callback`, `return`)
        VALUES ('$serverEsc','$pemilikEsc','$vaEsc','$apiKeyEsc','$pajakEsc','$bhpsUsoEsc','$authModeEsc','$callbackEsc','$returnEsc')
        ON DUPLICATE KEY UPDATE `va`=VALUES(`va`), `api_key`=VALUES(`api_key`), `pajak`=VALUES(`pajak`), `bhps_uso`=VALUES(`bhps_uso`),
...
if ($query) {
    ...
    respond_and_exit('Gagal menyimpan pengaturan: ' . $error . ' (Pastikan tabel `ipaymu` sudah dibuat, lihat migrations/2026-07-17_add_ipaymu_doku_faspay.sql)');
}
