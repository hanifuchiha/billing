...
if (!in_array($nominalType, ['nominal', 'persentase'], true)) {
    json_out(false, 'Jenis nilai diskon tidak valid.');
}
if (!in_array($periodeType, ['bulanan', 'rentang', 'permanen'], true)) {
    json_out(false, 'Jenis periode diskon tidak valid.');
}
if ($nominal <= 0) {
    json_out(false, 'Nilai diskon harus lebih dari 0.');
}
...
if ($periodeType === 'bulanan') {
    if (!in_array($periodeMonth, $bulan_penggunaan, true) || $periodeYear < 2000 || $periodeYear > 2100) {
        json_out(false, 'Periode diskon tidak valid.');
    }
...
if (!$stmtInsert) {
    json_out(false, 'Gagal menyiapkan query simpan diskon.');
}

$stmtInsert->bind_param('sssssssdss', $idpel, $pemilik, $periode, $periodeType, $periodeMulai, $periodeSelesai, $nominalType, $nominal, $keterangan, $createdBy);
$okInsert = $stmtInsert->execute();
$stmtInsert->close();

if (!$okInsert) {
    json_out(false, 'Gagal menyimpan diskon pelanggan.');
}

json_out(true, 'Diskon pelanggan berhasil disimpan.');
