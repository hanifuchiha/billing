...
$headers = ['pool_name', 'ipawal', 'ipakhir', 'iplocal', 'pemilik'];
$example = ['Pool-AREA01', '192.168.10.0/24', '', '192.168.10.1', $ceknama];
$notes = [
    'Nama unik pool',
    'Bisa IP awal atau subnet (contoh: 192.168.10.0/24)',
    'Isi jika ipawal bukan subnet',
    'IP lokal (gateway)',
    'Kosongkan untuk pakai user login'
];
...
$tempDir = sys_get_temp_dir() . '/excel_template_pool_' . uniqid();
@mkdir($tempDir);
@mkdir($tempDir . '/_rels');
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <dc:title>Template Import IP Pool</dc:title>
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
    <sheets>
        <sheet name="Template Pool" sheetId="1" r:id="rId1"/>
...
for ($i = 0; $i < count($headers); $i++) {
    ...
    $worksheetXml .= '<c r="' . $col . '1" t="inlineStr"><is><t>' . htmlspecialchars($headers[$i], ENT_QUOTES | ENT_XML1, 'UTF-8') . '</t></is></c>';
}
$worksheetXml .= '</row>';
...
for ($i = 0; $i < count($example); $i++) {
    $col = getExcelColumnName($i);
    $worksheetXml .= '<c r="' . $col . '2" t="inlineStr"><is><t>' . htmlspecialchars($example[$i], ENT_QUOTES | ENT_XML1, 'UTF-8') . '</t></is></c>';
}
$worksheetXml .= '</row>';
...
for ($i = 0; $i < count($notes); $i++) {
    $col = getExcelColumnName($i);
    $worksheetXml .= '<c r="' . $col . '3" t="inlineStr"><is><t>' . htmlspecialchars($notes[$i], ENT_QUOTES | ENT_XML1, 'UTF-8') . '</t></is></c>';
}
$worksheetXml .= '</row>';
...
header('Content-Disposition: attachment; filename="template_import_pool.xlsx"');
