<?php
/**
 * proses/sync_packageshotspot.php
 *
 * Menerima Hotspot User Profile MikroTik yang dicentang user di modal Sync Profile,
 * lengkap dengan harga & uptime yang dipilih, lalu insert ke tabel `paket_hotspot`.
 *
 * Request: POST
 *   - ip          : IP server mikrotik (utk lookup data server)
 *   - pemilik     : username mikrotik (kolom pemilik)
 *   - profiles    : JSON array, tiap item:
 *       { name, rate_limit, uptime, harga, komisi }
 *
 * Redirect ke ../packageshotspot.php dengan ?statusnotif=sync_success / sync_failed
 */

// --- Pastikan TIDAK PERNAH muncul HTTP 500 blank. Apapun errornya,
//     redirect balik ke packageshotspot.php dengan pesan error yang jelas. ---
error_reporting(E_ALL);
ini_set('display_errors', '0');

function redirect_failed(string $msg) {
    header('Location: ../packageshotspot.php?statusnotif=sync_failed&text=' . urlencode($msg));
    exit;
}
...
register_shutdown_function(function () {
    ...
    if ($err && in_array($err['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR], true)) {
        if (!headers_sent()) {
            header('Location: ../packageshotspot.php?statusnotif=sync_failed&text=' . urlencode(
...
foreach ($profiles as $p) {
    ...
    $sqlDup  = "SELECT id FROM paket_hotspot WHERE pemilik = '$pemilikEsc' AND paket = '$nameEsc' LIMIT 1";
    ...
    $sqlInsert = "INSERT INTO paket_hotspot (paket, uptime, ratelimit, harga, komisi, area, pemilik, BRAND)
                  VALUES (
                    '" . $nameEsc . "',
                    '" . mysqli_real_escape_string($conn, $uptime) . "',
...
if ($inserted > 0 && count($errors) === 0) {
    header('Location: ../packageshotspot.php?statusnotif=sync_success&synced=' . $inserted . '&skipped=' . $skipped);
} elseif ($inserted > 0 && count($errors) > 0) {
    header('Location: ../packageshotspot.php?statusnotif=sync_success&synced=' . $inserted . '&skipped=' . $skipped . '&partial_error=1');
} else {
