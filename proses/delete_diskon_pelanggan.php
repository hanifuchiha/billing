...
$id = (int)($_POST['id'] ?? 0);
if ($id <= 0) {
    json_out(false, 'ID diskon tidak valid.');
}
...
$sql = "UPDATE diskon_pelanggan SET ACTIVE = 0 WHERE id = ? AND PEMILIK IN ($server_list)";
$stmt = $conn->prepare($sql);
if (!$stmt) {
    json_out(false, 'Gagal menyiapkan query hapus diskon.');
}
...
if ($affected > 0) {
    json_out(true, 'Diskon berhasil dihapus.');
}

json_out(false, 'Data diskon tidak ditemukan atau tidak punya akses.');
