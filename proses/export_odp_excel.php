...
if($AKSES != 'ASSISTANT') {
  ...
  $sql = "SELECT * FROM odp WHERE `PEMILIK` IN ($userServerList) ORDER BY id DESC";
} else {
  $sql = "SELECT * FROM odp WHERE `AREA` IN ($area_list) ORDER BY id DESC";
}
...
$rows = [];
$rows[] = ['KODE', 'NAME', 'TIKOR', 'PEMILIK', 'AREA', 'BRAND', 'HIRARKI', 'SPLITTER', 'HIRARKI_PARENT'];
...
$tempDir = sys_get_temp_dir() . '/xlsx_export_odp_' . uniqid();
@mkdir($tempDir);
@mkdir($tempDir . '/_rels');
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="ODP Export" sheetId="1" r:id="rId1"/>
  </sheets>
</workbook>');
...
$filename = 'odp_export_' . date('Ymd_His') . '.xlsx';
...
$history[] = "[ " . (!empty($asistant_name) ? $asistant_name : $ceknama) . " - " . date('Y-m-d H:i:s') . " ] Export ODP";
