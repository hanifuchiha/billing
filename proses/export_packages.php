...
if ($AKSES != 'ASSISTANT') {
    ...
    $sql = "SELECT * FROM paket WHERE `PEMILIK` IN ($userServerList) ORDER BY id DESC";
} else {
    // Untuk ASSISTANT, filter berdasarkan area yang diizinkan
    $sql = "SELECT * FROM paket WHERE `AREA` IN ($area_list) ORDER BY id DESC";
}
...
$rows = [];
$rows[] = ['Nama', 'Kecepatan', 'Harga', 'Local', 'Remote', 'PEMILIK', 'Area', 'Komisi', 'Brand'];
...
// Build XLSX in temp directory
$tempDir = sys_get_temp_dir() . '/xlsx_export_paket_' . uniqid();
@mkdir($tempDir);
@mkdir($tempDir . '/_rels');
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <dc:title>Export Paket PPPoE</dc:title>
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
    <sheets>
        <sheet name="Paket PPPoE" sheetId="1" r:id="rId1"/>
    </sheets>
</workbook>');
...
$filename = 'paket_pppoe_' . date('Ymd_His') . '.xlsx';
...
$history[] = "[ " . (!empty($asistant_name) ? $asistant_name : $ceknama) . " - " . date('Y-m-d H:i:s') . " ] Export paket PPPoE";
