...
// Dipindah ke atas (sebelum baca JSON reminder & pilih bot) supaya jalur gagal
// di bawah (JSON hilang/rusak, bot tidak ketemu) bisa dicatat ke history --
// sebelumnya cuma di-echo() dan kebuang oleh redirect cron `> /dev/null 2>&1`.
...
if (file_exists($jsonFile)) {
  ...
  if ($data !== null) {
    ...
    $history[] = "[ system billing - " . date('Y-m-d H:i:s') . " ] GAGAL TOTAL: file reminder-$pemilik.json ada tapi tidak bisa didecode (JSON rusak) - cron notif_cek_servernotif dihentikan";
    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
    die();
  }
} else {
  echo "Error: File JSON tidak ditemukan.";
  $history[] = "[ system billing - " . date('Y-m-d H:i:s') . " ] GAGAL TOTAL: file reminder-$pemilik.json tidak ditemukan - cron notif_cek_servernotif dihentikan";
  file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
  die();
}
...
if ($bot_result['success']) {
  ...
  if ($bot_result['is_random']) {
    echo "[RANDOM BOT] Bot dipilih secara acak: $botname untuk menghindari spam (Server Check)<br>";
  }
} else {
  echo "ERROR: " . $bot_result['message'] . "<br>";
  $history[] = "[ system billing - " . date('Y-m-d H:i:s') . " ] GAGAL TOTAL: tidak ada bot WA ditemukan untuk pemilik $pemilik (" . $bot_result['message'] . ") - cron notif_cek_servernotif dihentikan";
  file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
  die();
}
