...
if (!empty($server_tidak_konek)) {
  ...
  if ($waOk) {
    ...
    $history[] = "[ system billing - " . date('Y-m-d H:i:s') . " ] Notif server tidak konek dikirim ke owner | HTTP: $httpCode";
  } else {
    echo "GAGAL mengirim notifikasi server tidak konek. HTTP: $httpCode, cURL error: $curlErr<br>";
    $history[] = "[ system billing - " . date('Y-m-d H:i:s') . " ] GAGAL kirim notif server tidak konek | HTTP: $httpCode | cURL error: $curlErr | Response: " . substr((string)$response, 0, 200);
  }
