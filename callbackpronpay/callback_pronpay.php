...
if (strtoupper($botname) == 'RANDOM') {
    ...
    if (mysqli_num_rows($query1) > 0) {
        ...
        while ($data1 = mysqli_fetch_array($query1)) {
            $availableBots[] = ['namebot' => $data1['namebot'], 'addressbot' => $data1['addressbot'], 'password' => $data1['password']];
        }
    ...
    $query1 = mysqli_query($conn, $sql1);
    if (mysqli_num_rows($query1) > 0) {
        while ($data1 = mysqli_fetch_array($query1)) { $waapi = $data1['addressbot']; $botpass = $data1['password']; $sender = $data1['sender'] ?? ''; }
    } else { $waapi = ''; $botpass = ''; $sender = ''; }
}
/////////////////////////////////////////////////////////////////////////////////
...
// ================= PRONPAY CONFIG & SIGNATURE =================
$sql = "SELECT * FROM pronpay WHERE pemilik='$ownerbilling'";
$query = mysqli_query($conn, $sql);
$data = mysqli_fetch_array($query);
if (!$data) {
    die(json_encode(["status" => "error", "message" => "Gagal mengambil data pronpay"]));
}
$apiKey = $data['apikey'];
...
$customer_phone = $arr['customer']['phone'] ?? '';
$payment_link = $arr['checkout_url'] ?? '#';
$sku = $arr['items'][0]['sku'] ?? '';
$invoiceref = $merchant_ref;
...
$history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] Pronpay cek kode ref transaksi $invoiceref";
...
// portal_bayar.php dari invoice yang benar). Fallback di atas cuma dipakai utk baris yang
// kebetulan belum punya PENGUNAAN (mis. bukan dari portal_bayar.php).
if ($pendingPengunaan !== '') {
    $periode = $pendingPengunaan;
}

////SESUAIKAN HARGA DARI PRONPAY ///
$HARGAPELANGGAN = $amount;
...
if (!in_array($PAKET, ["TOPUP", "TOPUP_MITRA"])) {
    ...
    /// => APAKAH PAKET VPN ?
    if (empty($LAYANAN)) { // Only check if not already found
        $sql112 = "SELECT * from `paket_vpn` WHERE `paket` = '$PAKET' ";
    ...
    /// => CEK APAKAH PAKET TIDAK DITEMUKAN DI MANA PUN
    if (empty($LAYANAN)) {

        // Log error ke history
        $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] ERROR: Paket '$PAKET' tidak ditemukan di database untuk transaksi $invoiceref";
        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));

        // Update status transaksi menjadi GAGAL
        $sql_error = "UPDATE `transaksi` SET `STATUS`='GAGAL', `CEK`='PAKET TIDAK DITEMUKAN' WHERE `BUKTI`='$invoiceref'";
        $conn->query($sql_error);

        // Kirim notifikasi error
        $to = $WHATSAPPELANGGAN ?? '6287740317266@s.whatsapp.net';
        $text = "[ERROR] Paket '$PAKET' tidak tersedia di sistem. Silakan hubungi admin untuk informasi lebih lanjut. Ref: $invoiceref";

        $data = ["phone" => $to, "message" => prependDynamicGreeting($text)];
        ...
        $url = "$waapi/send/message?session=$session";
        if ($deviceId !== '') {
        ...
        die(json_encode(["status" => "error", "message" => "Paket tidak ditemukan di database"]));
    }
} else {
    $LAYANAN = $PAKET;
}
...
if (!in_array($PAKET, ["TOPUP", "TOPUP_MITRA"])) {
    ...
    if (mysqli_num_rows($query_server) == 0) {
        // Log error ke history
        $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] ERROR: Server tidak ditemukan untuk area '$AREAPELANGGAN' - transaksi $invoiceref";
        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
    ...
    if ($LAYANAN == "PPPOE") {
            ...
            $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] Pronpay berhasil notif BOT wahtsapp dan email pelanggan  $text";
            // Simpan ke file history
            file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
            ...
            if ($conn->query($sql1) === TRUE) {
                ...
                if ($conn->query($sql11) === TRUE) {
                    ...
                    $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] Pronpay berhasil mencatat transaksi OTOMATIS  '$tanggalbayar','$periode','$USERNAMETRANASAKSI','$NAMAPELANGGAN','$PAKETPELANGGAN','$HARGAPELANGGAN','BERHASIL','$invoiceref','$BRANDPELANGGAN',''";
                    // Simpan ke file history
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
...
if($AUTHMODE=='API MODE' | $AUTHMODE=='MULTI MODE'  )
{
                    ...
                    $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] Pronpay berhasil aktifkan OTOMATIS  '$USERNAMETRANASAKSI','$NAMAPELANGGAN','$PAKETPELANGGAN'";
                    // Simpan ke file history
                    file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
...
if($AUTHMODE=='RADIUS MODE' | $AUTHMODE=='MULTI MODE'  )
{
                                    ...
                                    if ($PASSWORDPELANGGAN === '' || $PASSWORDPELANGGAN === null) {
                                        $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] PERINGATAN: password PPPoE kosong untuk $USERNAMETRANASAKSI, RADIUS TIDAK diaktifkan otomatis.";
                                    } else {
            ...
            if ($conn->query($sql10) === TRUE) {
                ...
                if ($sekali == 1) {
                    ...
                    if ($conn->query($sql13) === TRUE) {
                        ...
                        $history[] = "[ callback pronpay - " . date('Y-m-d H:i:s') . " ] ERROR: Gagal update transaksi topup $invoiceref - " . $conn->error;
                        file_put_contents($history_file, json_encode($history, JSON_PRETTY_PRINT));
                    }
                    $sekali = $sekali + 1;
                }
