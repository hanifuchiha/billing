...
foreach ($servers as $srv) {
    ...
    // Speed up scan: avoid long retries for offline routers.
    $api->timeout = 2;
    $api->attempts = 1;
    $api->delay = 0;
    ...
    $api->write('=.proplist=name,profile,service,disabled,last-logged-out,last-caller-id,comment');
    $read = $api->read(false);
    ...
    foreach ($secrets as $secret) {
        ...
        $hasil[] = [
            ...
            'last_logged_out' => (string)($secret['last-logged-out'] ?? '-'),
            'last_caller_id' => (string)($secret['last-caller-id'] ?? '-'),
...
$responsePayload = [
    ...
    'total_secret_checked' => $totalSecrets,
