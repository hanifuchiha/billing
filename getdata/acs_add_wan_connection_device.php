...
$object_name = trim((string)($data['object_name'] ?? ''));

if ($server_id <= 0 || $serial_raw === '' || $object_name === '') {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Payload tidak lengkap']);
    exit;
}

if ($object_name !== 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.') {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Object WANConnectionDevice tidak valid']);
    exit;
}
...
if ($http_code < 200 || $http_code >= 300) {
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Gagal kirim task addObject WAN ke ACS',
        'http_code' => $http_code,
        'error' => $curl_err,
        'response' => $response,
    ]);
...
echo json_encode([
    'success' => true,
    'message' => 'Task tambah WANConnectionDevice berhasil dikirim ke ACS',
