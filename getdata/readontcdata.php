...
if(isset($_GET['fetch'])){
    ...
    $onu_oids = [
        ...
        'onuDistance' => '1.3.6.1.4.1.17409.2.8.4.1.1.9',
        'onuTemperature' => '1.3.6.1.4.1.17409.2.8.4.1.1.10',
        'onuTxPower' => '1.3.6.1.4.1.17409.2.8.4.1.1.11',
        'onuRxPower' => '1.3.6.1.4.1.17409.2.8.4.1.1.12',
    ];
    ...
    foreach($onu_rows as $idx => $data){
        $onu_array[] = [
            ...
            '', // CHIP
            '', // PORTNUM
            $data['onuDistance'] ?? '', // Distance
            $data['onuTemperature'] ?? '', // Temperature
            $data['onuTxPower'] ?? '', // Tx Power
            $data['onuRxPower'] ?? '', // Rx Power
            ''  // Additional
        ];
    }
    ...
    file_put_contents($debug_dir.'/onulist.txt', json_encode($onu_array, JSON_PRETTY_PRINT));
...
<head>
<meta charset="utf-8">
<title>CDATA GPON ONU Auto Manager - SNMP</title>
...
<h5 class="mb-0">Login to CDATA GPON ONU Manager</h5>
</div>
<div class="card-body">
<form method="post">
<div class="mb-3">
...
<h5 class="mb-0">CDATA GPON ONU Auto Manager - SNMP</h5>
    ...
    fetch('?fetch=1')
        ...
        .then(data => {
            ...
            data.onu.forEach((r,i) => {
                let status = r[3] || '';
                let cls = status.toLowerCase().includes('up') ? 'table-success' : 'table-danger';
                let dist = parseFloat(r[8]||0) * 1.6393;
